...
DATASET="${DATASET:-data/screen-structure-rich-v5}"
PRESET="${PRESET:-balanced}"
BASE_MODEL="${BASE_MODEL:-models/yolo_screen_structure_rich_v4_quick_local.pt}"
RUN_NAME="${RUN_NAME:-yolo_screen_structure_rich_v5}"
EPOCHS="${EPOCHS:-80}"
IMGSZ="${IMGSZ:-1280}"
BATCH="${BATCH:-4}"
DEVICE="${DEVICE:-mps}"
...
if [[ "$BUILD_DATASET" == "1" || ! -f "$DATASET/data.yaml" ]]; then
  python scripts/build_yolo_rich_dataset.py \
    --out "$DATASET" \
    --tmp "data/_tmp_$(basename "$DATASET")" \
    --preset "$PRESET" \
    --clean
fi
